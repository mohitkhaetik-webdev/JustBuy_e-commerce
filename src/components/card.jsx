    ...
    <div className='lg:px-0 px-5' onClick={() => { dispatch(showdetails({ img, price, name, id, qty, description })); navigate('/Productdetails') }} >
      <div className=' max-w-[250px]  space-y-2 '>
        ...
        <div className='flex justify-between items-center'>
          <div id='addtocart' className='bg-red-500 text-center p-1 text-sm   font-medium text-white rounded-lg cursor-pointer' onClick={(e) => {dispatch(Additem({ id, name, img, price, qty, description })); toast("Your item is added"); e.stopPropagation() }}>ADD TO CART</div>
          <span className='rounded-md bg-gray-300 text-center  p-1 text-[10px] '>FREE DELIVERY</span>
        </div>
      </div>
    </div>
  )
